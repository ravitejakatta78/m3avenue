				...
				<form method="get" action="">
					...
					<div class="col-12 row">
						 ...
						 if(!empty($_GET['employeeid'])){
							...
							$sql = runloopQuery("SELECT * FROM clients where employee_id = '".$empid."' and employee_id in ('".$empString."')  and status = 'Pending' order by ID desc  LIMIT $page_position, $item_per_page");
							$totalordersarray = runQuery("SELECT count(*) as count FROM clients where employee_id = '".$empid."' and status = 'Pending' order by ID desc")['count'];	
							
						 }else if(!empty($_GET['clientmobile'])){ 
							 $empid = $_GET['clientmobile'];
							$sql = runloopQuery("SELECT * FROM clients where mobile = '".$empid."' and employee_id in ('".$empString."')  and status = 'Pending' order by ID desc  LIMIT $page_position, $item_per_page");
							$totalordersarray = runQuery("SELECT count(*) as count FROM clients where mobile = '".$empid."' and status = 'Pending' order by ID desc")['count'];	
						 }else{
							$sql = runloopQuery("SELECT * FROM clients where  employee_id in ('".$empString."') and status = 'Pending' order by ID desc  LIMIT $page_position, $item_per_page");
							$totalordersarray = runQuery("SELECT count(*) as count FROM clients where status = 'Pending' order by ID desc")['count'];	
						 }
					...
					<div class="kt-portlet__body">
					   ...
					   <div class="row form-group">
							<div class="col-12">
									...
									<select class="form-control" id="status<?php echo $row["ID"];?>" name="status"  data-clientid="<?php echo $row["ID"];?>">
										<option value="">--Select--</option>
...
<script>$("#myModal").modal('show');</script>
  ...
  <script>	
	 ...
	 var changestatus = $("#status"+checked).val();
	 $.ajax({				
