			...
			case 'getAttendanceDetails':
				if(!empty($_REQUEST['sdate']) && !empty($_REQUEST['edate'])){
					...
					foreach($sqlempdetails as $details){
						...
						$singledetail['total_hours'] = !empty($details['total_hours']) ? getHourMinuteFormat($details['total_hours']) : 0;
						$singledetail['total_break_hours'] = !empty($details['total_break_hours']) ? getHourMinuteFormat($details['total_break_hours']) : 0;
						$singledetail['effective_hours'] = !empty($details['effective_hours']) ? getHourMinuteFormat($details['effective_hours']) : 0;
						$breakdetail = [];
