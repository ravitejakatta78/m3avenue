	...
	<div class="kt-content  kt-grid__item kt-grid__item--fluid" id="kt_content">
		 ...
		 <form  method="post" action="" enctype="multipart/form-data" autocomplete="off" > 
                        ...
                        <div class="col-md-12">
							...
							<?php $campaignsmanagers = runloopQuery("select * from employee where ID in ('".$empString."') and status= 1 and role_id = '3' order by ID desc");
								foreach($campaignsmanagers as $campaigns){
							?>
							<option value="<?php echo $campaigns['ID'];?>"><?php echo $campaigns['fname'];?></option>
								<?php }?>
							</select>
                        </div> 
                    </div>
					...
					<thead>
						<tr>
							...
							<!--<th>Status:</th>-->
								...
								<tbody>
								  <?php
								  $empArr = array_column($tree,'ID');
$sql = runloopQuery("SELECT * FROM campaigns  order by ID desc");

   $x=1;  foreach($sql as $row)
		{
		if($row['executive']){
			$exeArray = explode(',',$row['executive']);
			$intersectedArray = array_intersect($exeArray,$empArr);
			if($intersectedArray) {
			...
			$managersnames =  runloopQuery("SELECT name FROM `employee` WHERE role_id = '3' and find_in_set_x('".$row['manager_id']."',ID)");
			if(!empty($managersnames)){
			$managersnames = array_column($managersnames, 'fname'); 
			}  
			}  
?>
<tr>
<td><?php echo  $x;?></td> 
<td><?php echo $row['user_role']=='superadmin' ? 'Admin' : ucwords(manager_details($row['user_id'],'fname'));?>
<br>
