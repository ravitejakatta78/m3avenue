...
if(!empty($_POST['fname']))
{
	...
	$pagerarray['leader'] = !empty($_POST['leader']) ? mysqli_real_escape_string($conn,$_POST['leader']) :  unique_id() ;
...
$superadmins_managers = runloopQuery("SELECT unique_id,concat(fname,' ',lname,'-',(case when role_id = 2 then 'Super Admin' else 'Operations Manager' end)) leadername  FROM employee where role_id in (2,5) and ID in ('".$empString."') order by ID desc");
$salesmanagers = runloopQuery("SELECT unique_id,concat(fname,' ',lname) leadername FROM employee where role_id = 3 and ID in ('".$empString."') order by ID desc");
$superadmins = runloopQuery("SELECT unique_id,concat(fname,' ',lname) leadername FROM employee where role_id = 2 and ID = '".$userid."' order by ID desc");
													...
													<div class="form-group row">
														...
														<div class="col-7">
															<select class="form-control"  name="filter_by_role" id="filter_by_role" >
																...
																<?php 
																foreach($roles as $roleid => $rolename) { if ($roleid != '1' && $roleid != '2' && $roleid != '5' && $roleid != '6' ) {?> 
											...
											<div class="table table-responsive">
												...
												<table class="table table-bordered table-hover table-checkable" id="kt_table_1">
													...
													<tbody>
														<?php
														if($filter_by_role!=''){
															$sql = runloopQuery("SELECT * FROM employee where role_id = '".$filter_by_role."' and ID in ('".$empString."')  order by ID desc");
														}else{
									...
									<div class="row">
										<div class="col-6">    
											...
											<select class="form-control"  name="role_id" id="role_id" onchange="rolebasedisplay()" required>
												...
												<?php foreach($roles as $roleid => $rolename) { if ($roleid != '1' && $roleid != '2' && $roleid != '5' && $roleid != '6' ) {?> 
